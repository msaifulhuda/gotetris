...
export default function Tetris() {
  ...
  const [currentPiece, setCurrentPiece] = useState<{ x: number; y: number; tetromino: { shape: number[][]; color: string } } | null>(null)
  ...
  const clearLines = useCallback((newBoard: number[][]) => {
    ...
    if (linesCleared.length > 0) {
      setCompletedRows(linesCleared as never[])
      setTimeout(() => {
  ...
  const renderCell = (x: number, y: number): string | number => {
    if (
      currentPiece &&
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      ...
      <div className="flex">
            ...
            {board.map((row, y) =>
              row.map((_, x) => (
                <AnimatePresence key={`${y}-${x}`}>
                  <motion.div
                    initial={{ opacity: completedRows.includes(y) ? 0 : 1, scale: completedRows.includes(y) ? 1.1 : 1 }}
                    animate={{ opacity: completedRows.includes(y) ? 0 : 1, scale: completedRows.includes(y) ? 1.1 : 1 }}
        ...
        <div className="ml-4">
          <div className="grid bg-gray-300" style={{ gridTemplateColumns: `repeat(${nextPiece.shape[0].length}, 1fr)`, width: `${nextPiece.shape[0].length * 20}px`, height: `${nextPiece.shape.length * 20}px`, border: '1px solid #e5e7eb' }}>
            {nextPiece.shape.map((row, y) =>
              row.map((value, x) => (
                <div key={`${y}-${x}`} className={`w-5 h-5 ${value ? `bg-${nextPiece.color}` : 'bg-gray-100'}`} style={{ border: '1px solid #e5e7eb' }} />
              ))
            )}
          </div>
        </div>
